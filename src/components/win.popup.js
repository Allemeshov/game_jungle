...
export class WinPopup extends Popup {
    ...
    win(seconds) {
        ...
        if (!newRating[localStorage.difficulty].includes(newRatingCard)) {
            newRating[localStorage.difficulty]
                .push(newRatingCard)
            localStorage.rating = JSON.stringify(newRating)
        }

        this.$el
            .querySelector('.game__board__score__amount')
            .innerHTML = SettingsStore.getTimeAmount() - seconds
        ...
        this.$el
            .querySelector('.game__board__bestScore__amount')
            .innerHTML = min
    }
