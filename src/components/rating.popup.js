...
export class RatingPopup extends Popup {
    ...
    onInit() {
        ...
        this.$el
            .querySelector('.rating__board__scroll__controller')
            .onmousedown = () => {
    ...
    getInfo() {
        ...
        Object.keys(_ratingCards).forEach(key => {
            _ratingCards[key].forEach(el => this.ratingCards.push(new RatingCardComponent(el.score)))
        })
    }
...
function scrollHandler() {
    if (
        ...
        this.$el
            .querySelector('.rating__board__scroll__controller')
            .style
            .top = this.$el.querySelector('.rating__board__scores').scrollTop
            / (
                this.$el.querySelector('.rating__board__scores').scrollHeight
                - this.$el.querySelector('.rating__board__scores').clientHeight
                + this.$el.querySelector('.rating__board__scroll__controller').clientHeight
            )
            * 100
            + '%'
    }
    ...
    if (
        ...
        let newY =
            (e.pageY - (
                ...
                + (this.$el.querySelector('.rating__board__scroll__controller').clientHeight / 2)
            ))
